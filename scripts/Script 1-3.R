...
# Pie femenino
labels1 <- c("Hombre", "Mujer")

dfpiemujeres <- df2 %>% 
  group_by(female) %>% # Variable to be transformed
  count() %>% 
  ungroup() %>% 
  mutate(perc = `n` / sum(`n`)) %>% 
  arrange(perc) %>%
...
ggplot(dfpiemujeres, aes(x = "", y = perc, fill = female)) +
  geom_col() +
  geom_text(aes(label = labels),
            position = position_stack(vjust = 0.5)) +
  coord_polar(theta = "y")+
  scale_fill_discrete(labels=labels1)+
  theme_void()+
  labs(fill = "")+
  ggtitle("Distribución de la muestra por sexo")+
...
ggsave("piemujeres.jpg")

# Pie formalidad
labels2 <- c("Informal", "Formal")

dfpieformal <- df2 %>% 
  group_by(formal) %>% # Variable to be transformed
  count() %>% 
  ungroup() %>% 
  mutate(perc = `n` / sum(`n`)) %>% 
  arrange(perc) %>%
...
ggplot(dfpieformal, aes(x = "", y = perc, fill = formal)) +
  geom_col() +
  geom_text(aes(label = labels),
            position = position_stack(vjust = 0.5)) +
  coord_polar(theta = "y")+
  ...
  theme_void()+
  labs(fill = "")+
  ggtitle("Distribución de la muestra por formalidad")+
  scale_color_viridis(option = "D")+
  theme(text = element_text(size = 12), plot.title = element_text(hjust = 0.5, size = 14), legend.text = element_text(size = 12), legend.title = element_text(size = 12))

ggsave("pieformal.jpg")

# Gráfico de area para el nivel educativo

df2_3 <- df2%>%subset(is.na(maxEducLevel) == FALSE)

ggplot(df2_3, aes(maxEducLevel))+
  geom_bar(aes(weight =fex_c), stat="count", width=0.7, fill="steelblue", group =1)+
  theme_classic()+
  scale_x_discrete(labels=c("1"="Ninguno", "3"="Primaria \n incompleta", 
  ...
  scale_color_viridis(option = "D")+
  theme(text = element_text(size = 11), plot.title = element_text(hjust = 0.5, size = 14))

ggsave("educ.jpg")

# Gráfico de area para el tamaño de la empresa
labels3 <- c("Cuenta propia", "2-5", "6-10", "11-50", 
             ">50")
...
ggplot(dfpie, aes(x = "", y = perc, fill = sizeFirm)) +
  ...
  geom_text(aes(label = labels),
            position = position_stack(vjust = 0.5)) +
  coord_polar(theta = "y")+
  scale_fill_discrete(labels=labels3)+
  theme_void()+
  labs(fill = "Empleados")+
...
ggplot(df2, aes(x = female , y = log_y)) +
  ...
  scale_color_manual(values = c("0"="darkred" , "1"="blue4") , label = c("0"="Hombre" , "1"="Mujer") , name = "Sexo")+
...
ggsave("inglabsex.jpg")

# Boxplot ingreso laboral por educación
dfinglabeduc <- df2%>%subset(is.na(maxEducLevel) == FALSE)

ggplot(dfinglabeduc, aes(x = maxEducLevel , y = log_y)) +
  geom_boxplot()+
  geom_point(aes(colour=maxEducLevel))+
  ggtitle("Ingreso laboral, por nivel de educación")+
  scale_color_manual(values = c("1", "3", "4", "5", "6", "7"), label = c("1"="Ninguno", "3"="Primaria \nincompleta", 
                     "4"="Primaria \ncompleta", "5"="Secundaria \nincompleta", 
                     "6"="Secundaria \ncompleta", "7"="Terciaria") , name = "Educación")+
...
ggsave("inglabeduc.jpg")

# Boxplot horas trabajadas por sexo
ggplot(df2, aes(x = female , y = hoursWorkUsual)) +
  geom_boxplot()+
  geom_point(aes(colour=female))+
  scale_color_manual(values = c("0"="darkred" , "1"="blue4") , label = c("0"="Hombre" , "1"="Mujer") , name = "Sexo")+
  ggtitle("Horas trabajadas, por sexo")+
  ylab("Horas trabajadas en la ocupación principal")+
  xlab("")+
  theme(plot.title = element_text(hjust = 0.5))+
  labs(fill = "Sexo")+
  theme_classic()+
...
ggsave("horastrabajadas.jpg")

# Boxplot horas trabajadas por sexo
ggplot(df2, aes(x = formal , y = log_y, fill=female)) +
  geom_boxplot()+
  geom_point(aes(colour=female))+
  scale_color_manual(values = c("0"="darkred" , "1"="blue4") , label = c("0"="Hombre" , "1"="Mujer") , name = "Sexo")+
  ggtitle("Horas trabajadas, por sexo")+
